...
A fully local streaming lakehouse: Kafka-style ingestion (Redpanda) → Parquet data lake (MinIO) → distributed SQL (Trino) → metadata catalog (Postgres) → FastAPI → SvelteKit UI. See [README.md](README.md) for the architecture diagram and [idea.md](docs/idea.md) / [pmo/adr/](pmo/adr/) for the "why" behind each technology choice.
